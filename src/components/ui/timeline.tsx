...
interface TimelineProps {
  data: TimelineEntry[];
  /** Optional heading rendered above the rail. Omit to render the rail alone. */
  heading?: React.ReactNode;
}

export const Timeline = ({ data, heading }: TimelineProps) => {
  const ref = useRef<HTMLDivElement>(null);
  const containerRef = useRef<HTMLDivElement>(null);
  const [height, setHeight] = useState(0);

  // The original measures once on mount, which leaves the progress beam short
  // when lazy content or web fonts land afterwards. Observe instead.
  useEffect(() => {
      ...
      <div ref={ref} className="relative mx-auto max-w-7xl pb-20">
        ...
        >
          <motion.div
            ...
            className="absolute inset-x-0 top-0 w-[2px] rounded-full bg-gradient-to-t from-yellow-500 from-[0%] via-amber-400 via-[10%] to-transparent"
          />
        </div>
      </div>
    </div>
